...
export class Guide extends Component {
  ...
  deleteClick(id) {
    axios
      .delete("http://localhost:8085/deleteguide/" + parseInt(id))
      .then((json) => {
        debugger;

        if (json.data == "Guide removed") {
      ...
      <div className="section-bg">
        ...
        <section id="main" className="d-flex align-items-center topMargin">
          <div className="container">
            <div>
              <h4 align="center">Guide Master</h4>
              <Link to={"/addguide"} className="btn btn-success btnAdd">
                Add
              </Link>
                ...
                <tbody>
                  {this.state.guide.map((item) => (
                    <tr key={item.guideId}>
                      ...
                      <td>{item.guideAddress}</td>
                      <td>{item.guideNumber}</td>
                      <td>{item.guidePrice}</td>
                      <td>
                        <Link
                          to={"/editguide?id=" + item.guideId}
                        ...
                        <button
                          ...
                          onClick={() => this.deleteClick(item.guideId)}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="16"
